#!/usr/bin/env python
"""
06_resnet_lstm_optuna.py — Optuna HPO for ResNet+LSTM, then final retraining.

Objective: minimise val RMSE_day (GHI >= day_threshold).

Usage (from project root):
    python scripts/06_resnet_lstm_optuna.py
    python scripts/06_resnet_lstm_optuna.py --site elpaso --seed 7 --n_trials 50
"""
...
# ------------------------------------------------------------------
# Optuna objective (closure over shared loaders / normalizer)
# ------------------------------------------------------------------
def make_objective(
    train_ds, val_ds,
