...
\begin{tikzpicture}[node distance=7mm]
  ...
  % ----------------------------------------------------------------
  % Row 1 (top): per-frame CNN encoder (shared weights across L=24 frames)
  % ----------------------------------------------------------------
  \node[inputblock] (input)
    {Input sequence\\ $L{=}24$ frames\\ \footnotesize $(C,P,P)=(16,16,16)$};

  \node[spatialblock, right=9mm of input] (stem) {Stem\\ Conv $3\times3$};
  ...
  % ----------------------------------------------------------------
  % Row 2 (bottom): LayerNorm -> sequence -> LSTM -> h_L -> MLP head -> y
  % Pushed down enough that the group labels below it don't collide
  % with the dashed encoder box above.
  % ----------------------------------------------------------------
  \node[neutralblock, below=24mm of input, text width=1.6cm] (ln) {Layer-\\Norm};
  ...
  \draw[flow] (res3) -- (gap);
  \draw[flow] (gap)  -- (lin);
  \draw[flow] (lin.south) |- ($(encbox.south)+(0,-6mm)$) -| (ln.north);
  \draw[flow] (ln) -- (s1);
  \draw[flow] (sL) -- (lstm);
  \draw[flow] (lstm) -- (hL);
  ...
  % Title
  % ----------------------------------------------------------------
  \node[font=\bfseries\normalsize, above=10mm of encbox.north] (title)
