...
\tikzset{
  ...
  pixnode/.style={circle, draw=cGraph!85!black, fill=cGraph!55, minimum size=2.2mm, inner sep=0pt},
...
\begin{tikzpicture}[node distance=7mm]
  ...
  \node[spatialblock, right=12mm of input, text width=2.3cm] (reshape)
    {Reshape to nodes\\ \footnotesize $N{=}256$ px, $C{=}16$ dims each};

  % ----------------------------------------------------------------
  % k-NN graph construction (built once over the 16x16 mesh) —
  % drawn as a small inset of nodes + inverse-distance-weighted edges
  % ----------------------------------------------------------------
  \node[graphblock, right=14mm of reshape, text width=2.5cm, minimum height=2.4cm] (graphbox)
    {};
  \node[font=\sffamily\footnotesize, align=center, anchor=north, yshift=-2mm] at (graphbox.north)
    {$k$-NN graph\\ (built once)};

  % small grid of pixel-nodes inside graphbox, with edges whose
  % opacity decays with distance (visual stand-in for 1/d(v,u) weights)
  \begin{scope}[shift={(graphbox.center)}, yshift=-1mm]
    \foreach \i in {0,1,2} {
      \foreach \j in {0,1,2} {
        \node[pixnode] (px-\i-\j) at (\i*3.2mm-3.2mm, \j*3.2mm-3.2mm) {};
      }
    }
    % nearest-neighbour edges (4-connectivity), opacity ~ inverse distance
    \foreach \i in {0,1,2} {
    ...
    % a couple of longer-range, fainter edges (further neighbours, lower weight)
  ...
  \end{scope}
  \node[dimlabel, anchor=north, yshift=-13mm] at (graphbox.north) {edge weight $\propto 1/d(v,u)$};

  % ----------------------------------------------------------------
  % GraphSAGE layers + readout
  % ----------------------------------------------------------------
  \node[graphblock, right=14mm of graphbox, text width=2.6cm] (sage)
    {$K\times$ GraphSAGE layer\\ \footnotesize weighted-mean aggr.\\
     concat(self,nbrs)$\to$Linear$\to$ReLU};

  \node[graphblock, right=8mm of sage, text width=2.3cm] (readout)
  ...
  % ----------------------------------------------------------------
  \node[neutralblock, below=18mm of reshape, text width=1.6cm] (ln) {Layer-\\Norm};
  ...
  \draw[flow] (readout.south) |- (ln.west);
  \draw[flow] (ln) -- (s1);
  \draw[flow] (sL) -- (lstm);
  \draw[flow] (lstm) -- (hL);
  ...
  % Title
  % ----------------------------------------------------------------
  \node[font=\bfseries\normalsize, above=22mm of sage] (title)
