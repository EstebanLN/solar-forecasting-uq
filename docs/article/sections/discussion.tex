...
\begin{itemize}
  \item For \textbf{El~Paso-type sites} (high irradiance, moderate variability),
        ...
        at 6h, reducing RMSE over persistence by approximately 60\%. At 3h, the
...
completing their Optuna runs (17 of the 24 site $\times$ horizon $\times$ seed
...
\textbf{No separate aleatoric uncertainty estimate.} This work quantifies
only the epistemic (reducible, data-dependent) share of predictive
uncertainty via the SGLD ensemble; it does not separately estimate the
aleatoric (irreducible) uncertainty arising from genuine cloud stochasticity,
e.g.\ via a heteroscedastic variance head. Consequently, the reported
interval $\bar\mu \pm z_{1-\alpha/2}\sigma_\text{epi}$ should be read as a
lower bound on total predictive uncertainty rather than a fully calibrated
interval; combining it with an explicit aleatoric component is a natural
direction for future work (Section~\ref{sec:conclusion}).
