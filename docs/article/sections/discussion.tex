...
\textbf{Aleatoric uncertainty as a proxy for cloud-regime volatility.} The
heteroscedastic variance head (Eq.~\ref{eq:nll}) is expected to assign larger
$\sigma_\text{ale}$ to inputs preceding rapid convective transitions ---
precisely the conditions identified in Section~\ref{sec:results} as driving
the largest forecast errors at both sites --- and smaller $\sigma_\text{ale}$
during stable clear-sky mornings, providing a continuously-varying,
input-dependent signal of intrinsic forecast difficulty that a constant
$\hat q$ cannot express.
...
of Sections~\ref{ssec:resnet}--\ref{ssec:ablation}.
...
completing their Optuna runs (16/24 of 24 site-horizon-seed configurations
evaluated at the time of writing). Final results will be incorporated to
quantify what fraction of deep-learning skill arises from spatial encoding
versus temporal dynamics alone.
