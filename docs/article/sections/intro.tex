...
history of observed GHI or satellite state \cite{paletta2021,wang2019}.
...
\cite{alessandrini2015,paletta2021}. More recently, graph neural networks
...
Two gaps thus remain open in the literature reviewed above: whether
graph-based spatial encoders offer any advantage over convolutional ones
under tropical Andean cloud dynamics, and whether posterior-sampling-based
epistemic uncertainty can reliably flag data-scarce forecasts in this same
setting. This paper addresses both, with the following contributions:
...
\begin{enumerate}
  \item We propose \textbf{GraphSAGE-LSTM}, a model that treats a GOES-16 satellite
        ...
        graph against a tuned, distance-weighted $k$-nearest-neighbour variant
        with a tunable neighbourhood size, isolating the contribution of
        \emph{tuned spatial connectivity} to forecasting skill.

  \item We conduct an \textbf{empirical comparison} across two climatologically
        ...
        for GraphSAGE-LSTM, ResNet-LSTM, and a FlatMLP spatial-ablation baseline,
        together with the graph-construction ablation described above; holding
        data, splits, temporal model, and training protocol fixed across
        architectures enables a controlled attribution of skill differences to
        the choice of spatial encoder.
...
Four-seed results (Section~\ref{sec:results}) indicate that GraphSAGE-LSTM is
\textbf{competitive with or superior to ResNet-LSTM} among the deep-learning
models at multi-hour horizons at both sites, while the absolute skill of both
models is markedly lower at the climatologically harder high-altitude site,
consistent with site-specific cloud dynamics shaping attainable forecasting
skill alongside model architecture. The FlatMLP spatial ablation and the
graph-construction ablation isolating tuned spatial connectivity are in
progress (Section~\ref{sec:results}).
