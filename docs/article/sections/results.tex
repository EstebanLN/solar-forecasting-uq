...
A notable observation from the FlatMLP spatial-ablation baseline (which
average-pools each satellite patch and so discards all within-patch spatial
structure) is that at Uniandes it is competitive with the spatially-aware
models at the longer horizons: at 6~hours FlatMLP reaches
$\skillday = 0.400$, above ResNet-LSTM ($0.368$) and just below
GraphSAGE-LSTM ($0.417$), and at 3~hours it is within a few points of both.
Only at 1~hour does a clear gap open ($0.077$ vs.\ ${\sim}0.15$). In other
words, at this tropical mountain site most of the achievable skill is
recoverable without any spatial encoding at all --- direct evidence for the
limited marginal value of satellite spatial features at Uniandes that we
analyse in Section~\ref{sec:discussion}.
...
\begin{table*}[t]
  ...
  \begin{tabular}{llcccccc}
    ...
    \midrule
    \multirow{2}{*}{FlatMLP}
      & $\rmseday$ & --- & --- & --- & $271.2 \pm 13.0$ & $275.5 \pm 5.6$ & $282.6 \pm 2.1$ \\
      & $\skillday$ & --- & --- & --- & $+0.077 \pm 0.044$ & $+0.320 \pm 0.014$ & $+0.400 \pm 0.004$ \\
    \bottomrule
  \end{tabular}}
  \par\smallskip
  ...
  Table~\ref{tab:ablation}. FlatMLP --- the spatial-ablation baseline
  (spatial average-pool, no spatial structure) --- is complete for Uniandes
  (four seeds at every horizon); its El~Paso row awaits the corrected
  retraining. This row quantifies what fraction of deep-learning skill arises
  from spatial encoding versus temporal dynamics alone.}
\end{table*}
...
\begin{figure*}[t]
  ...
  \caption{Daytime skill score ($\skillday$) for ResNet-LSTM, GraphSAGE-LSTM
           ...
           to the other bars; FlatMLP (the spatial-ablation baseline) is
           reported in Table~\ref{tab:main} rather than plotted here. El~Paso's
...
\begin{figure*}[t]
  ...
  \caption{Daytime RMSE ($\rmseday$, W/m²) for ResNet-LSTM, GraphSAGE-LSTM,
           ...
           (Section~\ref{ssec:sarima}); FlatMLP (spatial-ablation baseline) is
           reported in Table~\ref{tab:main} rather than plotted here. As in
           Figure~\ref{fig:skill}, El~Paso's
...
\begin{table*}[t]
  \centering
  \caption{Epistemic uncertainty. $\sigma_\text{epi}$ in W/m²
           ...
           at 3h and 6h). \textit{[Pending: the epistemic estimator is not yet
           finalised --- a fixed-step SGLD chain drifts over the full sampling
           budget (Section~\ref{ssec:uq}, Open issue). Values will be populated
           once the estimator is settled.]}}
