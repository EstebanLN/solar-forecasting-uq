...
\begin{table*}[t]
  ...
  graph} variant (Section~\ref{ssec:gsage}); results for the learned weighted
  $k$-NN variant are in progress and reported separately in
  Table~\ref{tab:ablation}. FlatMLP (Optuna) results are pending completion of
...
\subsection{Graph-construction ablation: fixed vs.\ learned spatial connectivity}
\label{ssec:ablation}

Table~\ref{tab:ablation} isolates the contribution of \emph{learned spatial
connectivity} by comparing the fixed unweighted 8-neighbour graph (used for
the GraphSAGE-LSTM rows of Table~\ref{tab:main}) against the learned,
...
\subsection{Uncertainty decomposition: aleatoric vs.\ epistemic}
\label{ssec:uq_results}

Table~\ref{tab:decomposition} will report the aleatoric/epistemic
decomposition (Eq.~\ref{eq:decomposition}) for the best-performing backbone
at each site-horizon combination, together with the combined predictive
interval's empirical coverage at nominal levels 80\%, 90\%, and 95\%.
$\sigma_\text{ale}$ (W/m²) is the heteroscedastic-head estimate of irreducible
cloud-stochasticity noise, averaged over the $T$ SGLD posterior samples;
$\sigma_\text{epi}$ (W/m²) is the spread of the $T$ posterior point
predictions and shrinks as more representative training data is available;
their ratio $\sigma_\text{epi}/\sigma_\text{total}$ indicates what fraction of
total predictive uncertainty is, in principle, reducible by further training.
...
\begin{table*}[t]
  \centering
  \caption{Aleatoric/epistemic uncertainty decomposition. $\sigma_\text{ale}$,
           $\sigma_\text{epi}$, $\sigma_\text{total}$ in W/m² (Eq.~\ref{eq:decomposition});
           $\hat{\text{cov}}_\text{day}$: empirical daytime coverage of the combined
           interval $\bar\mu \pm z_{1-\alpha/2}\sigma_\text{total}$ on the test set.
           Backbone: GraphSAGE-LSTM for El~Paso; best-per-horizon deep-learning
           backbone for Uniandes (ResNet-LSTM at 1h, GraphSAGE-LSTM at 3h and 6h).
           \textit{[SGLD sampling and variance-head training in progress; values
           to be populated as runs complete.]}}
  ...
  \begin{tabular}{llccccc}
    \toprule
    \textbf{Site} & \textbf{H} &
    $\sigma_\text{ale}$ & $\sigma_\text{epi}$ & $\sigma_\text{total}$ &
    $\sigma_\text{epi}/\sigma_\text{total}$ & $\hat{\text{cov}}_\text{day}$ (90\%) \\
    \midrule
    El Paso  & 3h & --- & --- & --- & --- & --- \\
    El Paso  & 6h & --- & --- & --- & --- & --- \\
    \midrule
