...
site (order selected by grid search on validation AIC; no clear-sky
normalisation is applied, so the model targets the same physical quantity,
GHI in W/m$^2$, as every other model). Because a seasonal period of $m=144$
...
\begin{figure*}[t]
  ...
  \caption{GraphSAGE-LSTM architecture. Each satellite frame is reshaped into
           ...
           over a $k$-NN graph (fixed unweighted baseline or learned
...
variants, isolating the contribution of \emph{learned spatial connectivity}
...
\begin{itemize}
  ...
  \item \textbf{Learned weighted $k$-NN graph (proposed).} Each node connects
        to its $k$ closest pixels by Euclidean distance in 2-D pixel
        coordinates ($k \in \{4,8,12,16\}$, tuned per site/horizon,
        Section~\ref{ssec:optuna}), and every edge is assigned an
        inverse-distance weight $w_{vu} = 1/d(v,u)$
        (Eq.~\ref{eq:edge_weight} below). Larger $k$ values progressively add
        diagonal, two-step, and knight-move neighbours, so $k$ effectively
        controls the spatial receptive field of a single graph layer. Unlike
        the fixed baseline or a convolutional filter, this topology lets the
        model learn \emph{different} aggregation weights for
        differently-located pixels (e.g.\ patch centre vs.\ periphery), rather
        than sliding one shared template across the image, and lets Optuna
        select the neighbourhood size that best matches each site's cloud
        spatial scale.
\end{itemize}
...
ablation reference for the value of learned spatial connectivity.
...
winning configuration is then retrained with a longer epoch budget and
increased patience (ResNet-LSTM and GraphSAGE-LSTM: 30~epochs, patience~8;
FlatMLP: 35~epochs, patience~10) and evaluated on the held-out test set.
...
Mixed Precision (AMP) is enabled when a GPU is available. Each experiment is
replicated with four seeds
...
The best-performing backbone identified in Sections~\ref{ssec:resnet}--\ref{ssec:mlp}
is extended into a probabilistic model that quantifies \emph{epistemic}
...
$p(\theta \mid \mathcal{D})$ instead, we use Stochastic Gradient Langevin
Dynamics (SGLD, \cite{welling2011}), which injects calibrated Gaussian noise
into each gradient step:
\begin{equation}
  \theta_{t+1} = \theta_t - \epsilon_t \nabla_\theta \mathcal{L}(\theta_t)
  + \sqrt{2\epsilon_t}\,\eta_t, \qquad \eta_t \sim \mathcal{N}(0, I),
  \label{eq:sgld}
\end{equation}
where $\epsilon_t$ is the (decaying) step size. Under suitable conditions on
$\epsilon_t$, the resulting Markov chain asymptotically samples from
$p(\theta \mid \mathcal{D})$ without the computational overhead of full MCMC.
Following a burn-in period of $T_\text{burn-in}$ epochs (discarded, while the
chain moves away from the SGD initialisation), we collect $T$ weight
snapshots $\{\theta_t\}_{t=1}^{T}$ at a fixed sampling interval
$\Delta$ (epochs), yielding an inexpensive approximate ensemble.
