...
The fixed-graph GraphSAGE-LSTM baseline, the corresponding ResNet-LSTM
configuration, and FlatMLP use a budget of 50 trials per configuration
(site $\times$ horizon $\times$ seed) across four random seeds
$\{42,1,7,13\}$. The extended search space used for the weighted $k$-NN
ablation (Table~\ref{tab:hpspace}) increases per-trial cost substantially ---
larger embeddings, optionally deeper LSTMs, and a tunable graph rebuilt per
trial --- so this variant uses a reduced budget of 75 trials across two
random seeds $\{42,1\}$, prioritising coverage of the most consequential
hyperparameters (graph connectivity, model capacity) over seed count; results
are reported with this caveat wherever the two protocols are compared
...
\subsection{Uncertainty quantification: disentangling aleatoric and epistemic uncertainty}
\label{ssec:uq}

The best-performing backbone identified in Sections~\ref{ssec:resnet}--\ref{ssec:mlp}
is extended into a probabilistic model that separately estimates the two
sources of forecast uncertainty \cite{kendall2017}: \emph{aleatoric}
uncertainty, the irreducible stochasticity of cloud formation that would
remain even with infinite training data, and \emph{epistemic} uncertainty,
the reducible uncertainty due to limited training data and finite model
capacity. Reporting a single undifferentiated interval cannot distinguish a
forecast that is uncertain because the cloud field is genuinely
unpredictable from one that is uncertain because the model has not seen
similar conditions before; the two call for different
operational responses (Section~\ref{sec:intro}).

\textbf{Aleatoric uncertainty: heteroscedastic Gaussian head.}
The deterministic scalar output head (Sections~\ref{ssec:resnet}--\ref{ssec:mlp})
is replaced with a two-head output $(\mu_\theta(x), \log\sigma_\theta^2(x))$
predicting, respectively, the conditional mean and log-variance of $y \mid x$,
trained by minimising the Gaussian negative log-likelihood
\begin{equation}
  \mathcal{L}_\text{NLL}(\theta) =
  \frac{1}{2\sigma_\theta^2(x)} \bigl(y - \mu_\theta(x)\bigr)^2
  + \frac{1}{2}\log\sigma_\theta^2(x),
  \label{eq:nll}
\end{equation}
which lets the model assign larger predicted variance to inputs it judges
intrinsically noisier (e.g.\ rapidly evolving convective afternoons),
penalised by the log-variance term against trivially inflating $\sigma^2$
everywhere.
...
\textbf{Combining both into a total-uncertainty decomposition.}
For a test input $x$, each posterior sample $\theta_t$ produces a pair
$(\mu_t(x), \sigma_t^2(x))$ via the NLL head. The ensemble mean
$\bar\mu(x) = \frac{1}{T}\sum_{t=1}^T \mu_t(x)$ is the point forecast, and the
total predictive variance decomposes, by the law of total variance, into an
aleatoric and an epistemic term \cite{kendall2017}:
\begin{equation}
  \underbrace{\textstyle\frac{1}{T}\sum_{t=1}^{T}\bigl(\mu_t(x)-\bar\mu(x)\bigr)^2}
    _{\text{epistemic } \sigma^2_\text{epi}(x)}
  \;+\;
  \underbrace{\textstyle\frac{1}{T}\sum_{t=1}^{T} \sigma_t^2(x)}
    _{\text{aleatoric } \sigma^2_\text{ale}(x)}
  \;=\;
...
shrinks as more representative training data becomes available, even without
retraining the variance head. $\sigma^2_\text{ale}(x)$ instead reflects the
model's learned, input-dependent estimate of irreducible noise and would not
shrink with more data. We report both components separately, alongside the
combined predictive interval $\bar\mu(x) \pm z_{1-\alpha/2}\,\sigma_\text{total}(x)$,
and evaluate empirical coverage at the same nominal levels used in prior
versions of this pipeline ($1-\alpha \in \{80\%, 90\%, 95\%\}$) for
comparability, in addition to reporting the aleatoric/epistemic split itself
